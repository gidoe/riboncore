...
//TODO: use gossip option ("Transcript the Tablet") instead, if Ribon adds support.
        ...
        TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000))
    {
        if(!pCreature->IsHostileTo(pPlayer))
        {
            ...
            if(Spell)
                ...
                error_log("RSCR: go_ethereum_prison summoned creature (entry %u) but faction (%u) are not expected by script.", pCreature->GetEntry(), pCreature->getFaction());
        }
    }
