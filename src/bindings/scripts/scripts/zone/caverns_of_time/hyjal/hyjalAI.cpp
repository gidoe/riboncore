...
void hyjalAI::SummonNextWave(Wave wave[18], uint32 Count, float Base[4][3])
{
    ...
    if(!wave[Count].IsBoss)
    {
        ...
        {
            NextWaveTimer = 15000;
            debug_log("RSCR: HyjalAI: debug mode is enabled. Next Wave in 15 seconds");
        }
    }
    else
    {
...
void hyjalAI::UpdateWorldState(uint32 id, uint32 state)
{
    ...
    }else debug_log("RSCR: HyjalAI: UpdateWorldState, but PlayerList is empty");
}
...
void hyjalAI::HideNearPos(float x, float y)
{
    ...
    Ribon::AllFriendlyCreaturesInGrid creature_check(m_creature);
    Ribon::CreatureListSearcher<Ribon::AllFriendlyCreaturesInGrid> creature_searcher(m_creature, creatures, creature_check);
    TypeContainerVisitor
        <Ribon::CreatureListSearcher<Ribon::AllFriendlyCreaturesInGrid>,
...
void hyjalAI::RespawnNearPos(float x, float y)
{
    ...
    Ribon::RespawnDo u_do;
    Ribon::WorldObjectWorker<Ribon::RespawnDo> worker(m_creature, u_do);
