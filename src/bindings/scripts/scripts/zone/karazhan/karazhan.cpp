...
bool GossipSelect_npc_barnes(Player *player, Creature *_Creature, uint32 sender, uint32 action)
{
    ...
    switch(action)
    {
        ...
        case GOSSIP_ACTION_INFO_DEF+3:
            ...
            outstring_log("RSCR: player (GUID %i) manually set Opera event to EVENT_OZ",player->GetGUID());
            break;
        case GOSSIP_ACTION_INFO_DEF+4:
            player->CLOSE_GOSSIP_MENU();
            pBarnesAI->m_uiEventId = EVENT_HOOD;
            outstring_log("RSCR: player (GUID %i) manually set Opera event to EVENT_HOOD",player->GetGUID());
            break;
        case GOSSIP_ACTION_INFO_DEF+5:
            player->CLOSE_GOSSIP_MENU();
            pBarnesAI->m_uiEventId = EVENT_RAJ;
            outstring_log("RSCR: player (GUID %i) manually set Opera event to EVENT_RAJ",player->GetGUID());
            break;
    }
