...
void npc_escortAI::EnterEvadeMode()
{
    ...
    if (IsBeingEscorted)
    {
        m_bIsReturning = true;
        ReturnToLastPoint();
        debug_log("RSCR: EscortAI has left combat and is now returning to last point");
    }
    else
...
void npc_escortAI::UpdateAI(const uint32 diff)
{
            ...
            if (CurrentWP == WaypointList.end())
            {
                if(DespawnAtEnd)
                {
                    ...
                    if (m_bCanReturnToStart)
                    {
                        ...
                        debug_log("RSCR: EscortAI are returning home to spawn location: %u, %f, %f, %f", POINT_HOME, fRetX, fRetY, fRetZ);
                        return;
                    }
    ...
    if (IsBeingEscorted && PlayerGUID && !m_creature->getVictim() && !m_bIsReturning)
    {
        if (m_uiPlayerCheckTimer < diff)
        {
            ...
            {
                debug_log("RSCR: EscortAI Evaded back to spawn point because player/group was to far away or not found");
...
void npc_escortAI::MovementInform(uint32 type, uint32 id)
{
    ...
    {
        debug_log("RSCR: EscortAI has returned to original home location and will continue from beginning of waypoint list.");
...
void npc_escortAI::SetRun(bool bRun)
{
    if (bRun)
    {
        if (!m_bIsRunning)
            ...
            debug_log("RSCR: EscortAI attempt to set run mode, but is already running.");
    }
    else
    {
        if (m_bIsRunning)
            ...
            debug_log("RSCR: EscortAI attempt to set walk mode, but is already walking.");
    }
    m_bIsRunning = bRun;
}
...
void npc_escortAI::Start(bool bIsActiveAttacker, bool bRun, uint64 uiPlayerGUID, const Quest* pQuest, bool bInstantRespawn, bool bCanLoopPath)
{
    ...
    {
        debug_log("RSCR ERROR: EscortAI attempt to Start while in combat");
        return;
    }

    if (IsBeingEscorted)
    {
        error_log("RSCR: EscortAI attempt to Start while already escorting");
        return;
    }

    if (WaypointList.empty())
    {
        error_db_log("RSCR: EscortAI Start with 0 waypoints (possible missing entry in script_waypoint)");
        return;
    }
    ...
    if (m_bCanReturnToStart && m_bCanInstantRespawn)
        debug_log("RSCR: EscortAI is set to return home after waypoint end and instant respawn at waypoint end. Creature will never despawn.");

    if(m_creature->GetMotionMaster()->GetCurrentMovementGeneratorType() == WAYPOINT_MOTION_TYPE)
    {
        m_creature->GetMotionMaster()->MovementExpired();
        m_creature->GetMotionMaster()->MoveIdle();
        debug_log("RSCR: EscortAI start with WAYPOINT_MOTION_TYPE, changed to MoveIdle.");
    }

    //disable npcflags
    m_creature->SetUInt32Value(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_NONE);

    debug_log("RSCR: EscortAI started with %d waypoints. ActiveAttacker = %d, Run = %d, PlayerGUID = %d", WaypointList.size(), m_bIsActiveAttacker, m_bIsRunning, PlayerGUID);
    ...
    //Start WP
    m_creature->GetMotionMaster()->MovePoint(CurrentWP->id, CurrentWP->x, CurrentWP->y, CurrentWP->z );
    debug_log("RSCR: EscortAI Next WP is: %d, %f, %f, %f", CurrentWP->id, CurrentWP->x, CurrentWP->y, CurrentWP->z);
