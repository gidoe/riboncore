...
void ScriptedAI::DoPlaySoundToSet(WorldObject* pSource, uint32 uiSoundId)
{
    ...
    {
        error_log("RSCR: Invalid soundId %u used in DoPlaySoundToSet (Source: TypeId %u, GUID %u)", uiSoundId, pSource->GetTypeId(), pSource->GetGUIDLow());
        return;
    }
...
void ScriptedAI::DoResetThreat()
{
    ...
    {
        error_log("RSCR: DoResetThreat called for creature that either cannot have threat list or has empty threat list (m_creature entry = %d)", m_creature->GetEntry());
        return;
    }
...
void ScriptedAI::DoTeleportPlayer(Unit* pUnit, float fX, float fY, float fZ, float fO)
{
    ...
    {
        if(pUnit)
            error_log("RSCR: Creature %u (Entry: %u) Tried to teleport non-player unit (Type: %u GUID: %u) to x: %f y:%f z: %f o: %f. Aborted.", m_creature->GetGUID(), m_creature->GetEntry(), pUnit->GetTypeId(), pUnit->GetGUID(), fX, fY, fZ, fO);
        return;
    }
...
{
    Unit* pUnit = NULL;
    Ribon::MostHPMissingInRange u_check(m_creature, fRange, uiMinHPDiff);
    Ribon::UnitLastSearcher<Ribon::MostHPMissingInRange> searcher(m_creature, pUnit, u_check);
...
std::list<Creature*> ScriptedAI::DoFindFriendlyCC(float fRange)
{
    ...
    Ribon::FriendlyCCedInRange u_check(m_creature, fRange);
    Ribon::CreatureListSearcher<Ribon::FriendlyCCedInRange> searcher(m_creature, pList, u_check);
    m_creature->VisitNearbyObject(fRange, searcher);
    return pList;
}
...
{
    std::list<Creature*> pList;
    Ribon::FriendlyMissingBuffInRange u_check(m_creature, fRange, uiSpellid);
    Ribon::CreatureListSearcher<Ribon::FriendlyMissingBuffInRange> searcher(m_creature, pList, u_check);
    m_creature->VisitNearbyObject(fRange, searcher);
    return pList;
}
...
Player* ScriptedAI::GetPlayerAtMinimumRange(float fMinimumRange)
{
    ...
    CellPair pair(Ribon::ComputeCellPair(m_creature->GetPositionX(), m_creature->GetPositionY()));
    Cell cell(pair);
    cell.data.Part.reserved = ALL_DISTRICT;
    cell.SetNoCreate();

    Ribon::PlayerAtMinimumRangeAway check(m_creature, fMinimumRange);
    Ribon::PlayerSearcher<Ribon::PlayerAtMinimumRangeAway> searcher(m_creature, pPlayer, check);
    TypeContainerVisitor<Ribon::PlayerSearcher<Ribon::PlayerAtMinimumRangeAway>, GridTypeMapContainer> visitor(searcher);
...
Creature* GetClosestCreatureWithEntry(WorldObject* pSource, uint32 Entry, float MaxSearchRange)
{
    ...
    CellPair pair(Ribon::ComputeCellPair(pSource->GetPositionX(), pSource->GetPositionY()));
    Cell cell(pair);
    cell.data.Part.reserved = ALL_DISTRICT;
    cell.SetNoCreate();

    Ribon::NearestCreatureEntryWithLiveStateInObjectRangeCheck creature_check(*pSource, Entry, true, MaxSearchRange);
    Ribon::CreatureLastSearcher<Ribon::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(pSource, pCreature, creature_check);

    TypeContainerVisitor<Ribon::CreatureLastSearcher<Ribon::NearestCreatureEntryWithLiveStateInObjectRangeCheck>, GridTypeMapContainer> creature_searcher(searcher);
