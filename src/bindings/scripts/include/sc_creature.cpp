...
void ScriptedAI::DoTeleportPlayer(Unit* pUnit, float x, float y, float z, float o)
{
    ...
    {
        if(pUnit)
            error_log("RSCR: Creature %u (Entry: %u) Tried to teleport non-player unit (Type: %u GUID: %u) to x: %f y:%f z: %f o: %f. Aborted.", m_creature->GetGUID(), m_creature->GetEntry(), pUnit->GetTypeId(), pUnit->GetGUID(), x, y, z, o);
        return;
    }
...
{
    Unit* pUnit = NULL;
    Ribon::MostHPMissingInRange u_check(m_creature, range, MinHPDiff);
...
std::list<Creature*> ScriptedAI::DoFindFriendlyCC(float range)
{
    ...
    Ribon::FriendlyCCedInRange u_check(m_creature, range);
...
{
    std::list<Creature*> pList;
    Ribon::FriendlyMissingBuffInRange u_check(m_creature, range, spellid);
