    ...
    if(m_sourceGUID == m_target->GetGUID())
    {
        ...
        if( !source->hasUnitState(UNIT_STAT_ISOLATED) )
        {
            ...
            switch(m_areaAuraType)
            {
                ...
                {
                    Ribon::AnyFriendlyUnitInObjectRangeCheck u_check(source, caster, m_radius);
                    Ribon::UnitListSearcher<Ribon::AnyFriendlyUnitInObjectRangeCheck> searcher(source, targets, u_check);
                    source->VisitNearbyObject(m_radius, searcher);
                    break;
                }
                case AREA_AURA_ENEMY:
                {
                    Ribon::AnyAoETargetUnitInObjectRangeCheck u_check(source, caster, m_radius); // No GetCharmer in searcher
                    Ribon::UnitListSearcher<Ribon::AnyAoETargetUnitInObjectRangeCheck> searcher(source, targets, u_check);
                    source->VisitNearbyObject(m_radius, searcher);
                    break;
                }
...
void AuraEffect::HandleChannelDeathItem(bool apply, bool Real, bool /*changeAmount*/)
{
    if(Real && !apply)
    {
        ...
        if( spellInfo->EffectItemType[m_effIndex] == 6265 &&
            (victim->getLevel() <= Ribon::XP::GetGrayLevel(caster->getLevel()) ||
