    ...
    *data >> m_targetMask;
    ...
    if( m_targetMask & TARGET_FLAG_SOURCE_LOCATION )
    {
        ...
        *data >> m_srcX >> m_srcY >> m_srcZ;
        if(!Ribon::IsValidMapCoord(m_srcX, m_srcY, m_srcZ))
            return false;
    }
    ...
    if( m_targetMask & TARGET_FLAG_DEST_LOCATION )
    {
        ...
        *data >> m_destX >> m_destY >> m_destZ;
        if(!Ribon::IsValidMapCoord(m_destX, m_destY, m_destZ))
            return false;
...
void Spell::SearchAreaTarget(std::list<Unit*> &TagUnitMap, float radius, SpellNotifyPushType type, SpellTargets TargetType, uint32 entry)
{
    ...
    Ribon::SpellNotifierCreatureAndPlayer notifier(*this, TagUnitMap, radius, type, TargetType, entry, x, y, z);
...
WorldObject* Spell::SearchNearbyTarget(float range, SpellTargets TargetType)
{
    switch(TargetType)
    {
        ...
        {
            Unit *target = NULL;
            Ribon::AnyUnfriendlyUnitInObjectRangeCheck u_check(m_caster, m_caster, range);
            Ribon::UnitLastSearcher<Ribon::AnyUnfriendlyUnitInObjectRangeCheck> searcher(m_caster, target, u_check);
        ...
        {
            Unit *target = NULL;
            Ribon::AnyFriendlyUnitInObjectRangeCheck u_check(m_caster, m_caster, range);
            Ribon::UnitLastSearcher<Ribon::AnyFriendlyUnitInObjectRangeCheck> searcher(m_caster, target, u_check);
            m_caster->VisitNearbyObject(range, searcher);
            return target;
        }
    }
    ...
    else if(pushType)
    {
                ...
                switch (m_spellInfo->Id)
                {
                    ...
                    case 51328:
                                ...
                                && m_targets.getUnitTarget()->GetDisplayId() == m_targets.getUnitTarget()->GetNativeDisplayId())))
                        {
                            ...
                            WorldObject* result = FindCorpseUsing <Ribon::ExplodeCorpseObjectCheck> ();
        ...
        else
        {
            switch(cur)
            {
                ...
                case TARGET_OBJECT_AREA_DST:
                {
                    ...
                    Ribon::GameObjectInRangeCheck check(x, y, z, radius + 50);
                    std::list<GameObject*> goList;
                    Ribon::GameObjectListSearcher<Ribon::GameObjectInRangeCheck> searcher(m_caster, goList, check);
        ...
        if(!unitList.empty())
        {
            if(uint32 maxTargets = m_spellValue->MaxAffectedTargets)
            {
                ...
                Ribon::RandomResizeList(unitList, m_spellValue->MaxAffectedTargets);
            }
            else
            {
                switch (m_spellInfo->Id)
...
SpellCastResult Spell::CheckItems()
{
    ...
    {
        CellPair p(Ribon::ComputeCellPair(m_caster->GetPositionX(), m_caster->GetPositionY()));
        Cell cell(p);
        cell.data.Part.reserved = ALL_DISTRICT;
        ...
        TypeContainerVisitor<Ribon::GameObjectSearcher<Ribon::GameObjectFocusCheck>, GridTypeMapContainer > object_checker(checker);
        CellLock<GridReadGuard> cell_lock(cell, p);
        cell_lock->Visit(cell_lock, object_checker, *m_caster->GetMap());
