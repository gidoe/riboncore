 ...
 */
#ifndef RIBON_SPELLAURAS_H
#define RIBON_SPELLAURAS_H
...
//
// Other case choice: each code line moved under if(Real) check is Ribon speedup,
//      each setting object update field code line moved under if(Real) check is significant Ribon speedup, and less server->client data sends
//      each packet sending code moved under if(Real) check is _large_ Ribon speedup, and lot less server->client data sends
//
...
class RIBON_DLL_SPEC AuraEffect
{
    public:
...
class RIBON_DLL_SPEC PersistentAreaAuraEffect : public AuraEffect
{
    public:
